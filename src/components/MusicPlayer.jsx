...
import customIcons, {
  ShuffleButton,
  MinimizeIcon,
} from "../assets/svgs/PlayerCustomIcons.jsx";
  ...
  return (
    <>
      ...
      <MusicPlayerContainer
        playerRef={playerRef}
        className={`${MusicPlayerCSS.playerContainer} ${
          isMinimized ? MusicPlayerCSS.minimized : ""
        }`}
...
const MinimizeContainer = ({ isMinimized, handleToggleMinimize }) => {
  return (
    <div
      className={MusicPlayerCSS.minButtonContainer}
...
const CustomAudioPlayer = forwardRef(
  ({ currentTitle, currentArtist, ...props }, ref) => {
    return (
      <div className={MusicPlayerCSS.customAudioPlayer}>
        <div className={MusicPlayerCSS.detailsContainer}>
          <h2 className={MusicPlayerCSS.title}>{currentTitle}</h2>
          <p className={MusicPlayerCSS.artist}>{currentArtist}</p>
        </div>
        <AudioPlayer
          {...props}
          ref={ref}
          className={MusicPlayerCSS.audioPlayer}
        />
...
const AudioVolume = ({ audioRef }) => {
  ...
  return (
    <>
      <button className={MusicPlayerCSS.volumeButton} onClick={toggleMute}>
